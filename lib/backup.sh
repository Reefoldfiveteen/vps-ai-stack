...
USERNAME="${SETUP_USER:-}"
if [[ -z "$USERNAME" ]]; then
  # non-interactive mode (cron) — try config, else error
  if [[ ! -t 0 ]]; then
    if [[ -f "$HOME/backup/.backup.conf" ]]; then
      source "$HOME/backup/.backup.conf"
      USERNAME="${SETUP_USER:-}"
    fi
    if [[ -z "$USERNAME" ]]; then
      echo "[-] Cannot determine username in non-interactive mode." >&2
      echo "    Set SETUP_USER env var or run interactively." >&2
      exit 1
    fi
  else
...
do_full_backup() {
  ...
  # Auto upload to Google Drive (non-interactive) if gdrive ready
  if command -v gdrive &>/dev/null && run_as_user "gdrive files list --max 1 2>/dev/null" >/dev/null 2>&1; then
    local folder_id; folder_id="$(gdrive_folder_id)"
    if [[ -n "$folder_id" ]]; then
      info "Auto-uploading to Google Drive..."
      if run_as_user "gdrive files upload --parent '$folder_id' '$archive_path' 2>&1"; then
        ok "Auto-upload complete: $(basename "$archive_path") -> Google Drive /AI_Stack_Backup/"
      else
        warn "Auto-upload failed — will retry next time."
      fi
    fi
  fi
}
...
# Runs full backup (includes auto-upload + cleanup).
export PATH="/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:\$HOME/.npm-global/bin:\$HOME/.local/bin"
export SETUP_USER="$USERNAME"
